...
const apikey = "84b9da01bf6c73ca68f085908aba9278";
...
async function checkWeather(city) {
  try {
    ...
    if (cachedWeatherData) {
      ...
      // If cached data is available, parse and display it
...
// Get the clear local storage button element
...
clearLocalStorageBtn.addEventListener("click", function () {
  ...
  const confirmation = confirm("Are you sure you want to clear all local storage data?");
  
  if (confirmation) {
    // Clear all items from local storage
    localStorage.clear();
    
    // Provide feedback to the user that local storage has been cleared
